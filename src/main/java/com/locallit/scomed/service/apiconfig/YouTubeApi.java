...
public class YouTubeApi {
	...
	public CommentThreadListResponse fetchCommentData(Map<String, String> params) {
		String url = buildYouTubeCommentsUrl(params);
...
//		String apiUrl = "https://www.googleapis.com/youtube/v3/commentThreads?part=snippet&videoId="+videoId+"&key="+token+"&maxResults="+limit;
		...
		try {
			...
			CommentThreadListResponse resp =
			    mapper.readValue(data, CommentThreadListResponse.class);
			return resp;
		} catch (Exception e) {
			e.printStackTrace();
			return null;
    ...
    public String buildYouTubeCommentsUrl(Map<String, String> config) {
    	String baseUrl = BASE_URL;
    	StringBuilder url = new StringBuilder(baseUrl);
    	
		if (config.containsKey(VIDEO_ID)) {
			url.append(URL_VIDEO_ID_PARAM).append(config.get(VIDEO_ID));
		} else {
