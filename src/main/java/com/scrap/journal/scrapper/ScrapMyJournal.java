...
public class ScrapMyJournal {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		Thread.sleep(3000);
		List<String> results = s.extractSearchResults(resultsPath,20);
		logger.info(results);
...
//		driver.findElement(By.xpath("/html/body/div[5]/div[2]/div/div/div[2]/div/div/button[2]")).click();
	...
	public List<String> extractSearchResults(String listingXPath , int totalResults){
		List<String> results = new ArrayList<>();
		
		if(totalResults > 20 ) totalResults = 20;
        int j = 1;
        for(int i = 0; i < totalResults; i++) {
        	String link = driver.findElement(By.xpath(listingXPath.replace("${index}", String.valueOf(j)))).getAttribute("href");
        	results.add(link);
        	j = j +2;	
        }
	...
	public void extractInfoFromResults(List<String> results,String titlePath, String abstractPath, String authorsPath, String doiPath) throws InterruptedException {
		for(String result: results) {
        	driver.get(result);
        	Thread.sleep(5000);
        	try {
        		String doi  = driver.findElement(By.xpath(doiPath)).getText();
        	...
        	try {
        		String Abstract  = driver.findElement(By.xpath(abstractPath)).getText();
        	...
        	try {
        		String authors  = driver.findElement(By.xpath(authorsPath)).getText();
